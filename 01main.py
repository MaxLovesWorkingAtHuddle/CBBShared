...
parser.add_argument('-H', '--headless', action='store_true', help="Make everything headless.")
...
# review the new code. You have this new url shiz to look at. very cool but very idk. "create_task"
...
class MultiPageOddsMonitor:
    ...
    async def change_hos_url(self, new_url: str):
        """Dynamically change the HOS URL and reset headers."""
        print(f"\n🔄 Changing HOS URL to: {new_url}")
        # Reset headers so we capture the fresh ones for the new match IDs
        self.captured_hos_headers = None 
        
        try:
            await self.hos_page.goto(new_url, wait_until="domcontentloaded")
            print("✅ HOS URL updated. Waiting for new headers in the background...")
        except Exception as e:
            print(f"❌ Failed to change URL: {e}")
    ...
    async def monitor_loop(self, interval_seconds: int = 7):
        ...
        try:
            ...
            print("\n Background monitoring task stopped.")
        except Exception as e:
            print(f"\n Monitor loop encountered an error: {e}")
...
async def main():
    monitor = MultiPageOddsMonitor()
    monitor_task = None # Keep track of the background task

    try:
        await monitor.start(headless=args.headless)
        await monitor.wait_for_pages_ready()
        
        print("\n" + "="*60)
        print("🎮 COMMAND MENU:")
        print("  [m]        - Start background monitoring")
        print("  [stop]     - Stop background monitoring")
        print("  [url <X>]  - Change HOS Multi-view URL to <X>")
        print("  [Enter]    - Force single extraction")
        print("  [q]        - Quit program")
        print("="*60 + "\n")

        while True:
            # Wait for user input without blocking the asyncio event loop
            cmd = await asyncio.get_event_loop().run_in_executor(None, input, "> ")
            ...
            if action == 'q':
                ...
                if monitor_task and not monitor_task.done():
                    print("⚠️ Already monitoring in the background!")
                else:
                    print("🚀 Starting background monitoring...")
                    if args.inMem:
                        monitor_task = asyncio.create_task(monitor.IMmonitor_loop())
                    else:
                ...
                if len(parts) > 1:
                    new_url = parts[1]
                    await monitor.change_hos_url(new_url)
                else:
                    print("⚠️ Please provide a URL. Usage: url https://...")

            elif action == '':
                # If they just press enter, do a manual extraction
                res = await monitor.extract_all()
