...
async def fetch_hos_games():
    ...
    unique_games = {g['hos_event_id']: g for g in games}.values()
...
def auto_match_games(hos_games, oo_games):
    ...
    for hos_idx, hos_game in enumerate(hos_games):
        ...
        for oo_idx in unmatched_oo:
            ...
            # Direct Match Score
            s_direct = (similarity(hos_game['hos_home'], oo_game['oo_home']) + 
                        similarity(hos_game['hos_away'], oo_game['oo_away'])) / 2
            
            # Swapped Match Score
            s_swap = (similarity(hos_game['hos_home'], oo_game['oo_away']) + 
                      similarity(hos_game['hos_away'], oo_game['oo_home'])) / 2
...
def render_matches(matches, selected_idx, oo_games):
    ...
    for idx in range(start_idx, end_idx):
        ...
        h_text = f"{hos['hos_home']} vs {hos['hos_away']}"
        
        if oo:
            o_text = f"{oo['oo_home']} vs {oo['oo_away']}"
            link = "↔"
...
def render_footer(matches, selected_idx, oo_games, input_buffer):
    ...
    matched_oo_ids = {m['oo_game']['oo_id'] for m in matches if m['oo_game']}
    available_oo = [g for g in oo_games if g['oo_id'] not in matched_oo_ids]
    ...
    def save_data(self):
        ...
        for match in self.matches:
            if match['oo_game']:
                ...
                output.append({
                    'hos_event_id': h['hos_event_id'],
                    'hos_home': h['hos_home'], 'hos_home_id': h['hos_home_id'],
                    'hos_away': h['hos_away'], 'hos_away_id': h['hos_away_id'],
                    'oo_fixture_id': o['oo_id'],
                    'oo_game_id': o['oo_game_id'],
                    'oo_home': o['oo_home'], 'oo_home_abbr': o['home_abbr'],
                    'oo_away': o['oo_away'], 'oo_away_abbr': o['away_abbr'],
